...
This script analyzes Treasury bonds from the secondary market. It parses input data (CUSIP, price, yield) and calculates various metrics including Yield to Maturity (YTM) using the Bond Equivalent Yield (BEY) method. It fetches live quotes via `webull_bond_fetcher` by default, with `finra_bond_fetcher` available as a dormant fallback (`--source finra`). Can also be scheduled to run daily via `setup_daily_fetch_launchd.py`.

> **Dependencies**
> - Python 3.10 or later
> - requests (used by calculate_YTW, webull_bond_fetcher, and finra_bond_fetcher)
...
### bond_price_server
This script serves an interactive, multi-bond price comparison chart in the browser (the static page in `web/`), with checkboxes to show/hide individual bonds and preset time ranges (1M/3M/6M/1Y/5Y/All). It exposes a small JSON API that fetches each watchlist CUSIP's price/yield history from Webull server-side (avoiding browser CORS restrictions) and caches successful fetches to disk so a failed fetch for one CUSIP doesn't blank out the others. The bond list is configured in `data/json/bond_price_watchlist.json`.

> **Dependencies**
> - Python 3.10 or later
> - Standard library only (uses webull_bond_fetcher module)
...
### setup_daily_fetch_launchd
This script generates and installs the macOS LaunchAgent that runs `bond_market_analyzer.py --daily` on a schedule (10:00 local time by default). It's safe to re-run: it always regenerates the plist from the constants at the top of the file and reloads it, so rescheduling is just editing those constants and re-running.

> **Dependencies**
> - Python 3.10 or later (macOS only)
> - Standard library only

### webull_bond_fetcher
This script fetches near-real-time Treasury bond quotes (last price, bid/ask, yield, coupon) and historical price/yield series from Webull's public quote page by CUSIP, via a page-scrape rather than an official API. It's the active/default data source for `bond_market_analyzer.py` and `bond_price_server.py`.
